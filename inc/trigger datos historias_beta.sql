...
CREATE TRIGGER trigger_beneficiario after insert
	...
	INSERT INTO gallbo_cms.cmscouch_pages(template_id,page_title)
    VALUES(7, (SELECT lower(CONCAT_WS(' ',b.nombre, b.apellido)) as nombre
				FROM gallbo_markoptic.beneficiario_solicitud b
				WHERE b.id = @new_id));
    ...
    INSERT INTO gallbo_cms.cmscouch_data_text(page_id,field_id,value,search_value)
        VALUES(@id,7,(SELECT b.edad FROM gallbo_markoptic.beneficiario_solicitud b
						WHERE b.id = @new_id),
					 (SELECT b.edad FROM gallbo_markoptic.beneficiario_solicitud b
						WHERE b.id = @new_id));
    ...
    INSERT INTO gallbo_cms.cmscouch_data_text(page_id,field_id,value,search_value)
		VALUES(@id,20,(SELECT l.nombre
					FROM gallbo_markoptic.beneficiario_solicitud b
					join gallbo_markoptic.localidades l on b.ciudad = l.id
                    WHERE b.id = @new_id),
				  (SELECT l.nombre
					FROM gallbo_markoptic.beneficiario_solicitud b
					join gallbo_markoptic.localidades l on b.ciudad = l.id
    ...
    INSERT INTO gallbo_cms.cmscouch_data_text(page_id,field_id,value,search_value)
		VALUES(@id,21,(SELECT r.nombre
					FROM gallbo_markoptic.beneficiario_solicitud b
					join gallbo_markoptic.regiones r on b.estado = r.id
                    WHERE b.id = @new_id),
				  (SELECT r.nombre
					FROM gallbo_markoptic.beneficiario_solicitud b
					join gallbo_markoptic.regiones r on b.estado = r.id
	...
	INSERT INTO gallbo_cms.cmscouch_data_text(page_id,field_id,value,search_value)
		VALUES(@id,22,(SELECT p.nombre
					FROM gallbo_markoptic.beneficiario_solicitud b
					join gallbo_markoptic.paises p on b.pais = p.id
                    WHERE b.id = @new_id),
				  (SELECT p.nombre
					FROM gallbo_markoptic.beneficiario_solicitud b
					join gallbo_markoptic.paises p on b.pais = p.id
	...
	INSERT INTO gallbo_cms.cmscouch_data_text(page_id,field_id,value,search_value)
		VALUES(@id,19,(SELECT s.peticion
					FROM gallbo_markoptic.beneficiario_solicitud b
                    join gallbo_markoptic.solicitud s on b.id_solicitud = s.id
                    WHERE b.id = @new_id),
				   (SELECT s.peticion
					FROM gallbo_markoptic.beneficiario_solicitud b
                    join gallbo_markoptic.solicitud s on b.id_solicitud = s.id
	...
	UPDATE gallbo_markoptic.solicitud s SET s.id_page = @id WHERE s.id = @new_sol;
