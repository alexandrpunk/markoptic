<!-- pop up inicio -->
<div class="modal fade" id="itt" tabindex="-1" role="dialog" aria-labelledby="Testimonios">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header modal-mark modal-magenta">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title text-center" id="itt">Testimonios de nuestros beneficiarios</h4>
      </div>
      <div class="modal-body">
<div class="embed-responsive embed-responsive-16by9">
<iframe width="560" height="315" src="https://www.youtube.com/embed/yBtdaSWxdbk?rel=0&autoplay=1" frameborder="0" allowfullscreen></iframe>
<!-- no se debe eprder el ?rel=0&autoplay=1-->
</div>
      </div>
    </div>
  </div>
</div>